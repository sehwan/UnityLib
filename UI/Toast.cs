...
public class ToastAlert : MonoSingleton<ToastAlert>
{
    static float pos_y = 0.3f;
    public GameObject[] gos = new GameObject[7];
    public int idx;
    public int count;
    ...
    {
        StopAllCoroutines();
        GameObject prefab = Resources.Load<GameObject>("Prefabs/toast_alert");
        Transform canvas = GameObject.Find("UM").transform;
        for (int i = 0; i < gos.Length; i++)
        {
            gos[i] = Instantiate(prefab, Vector3.zero, Quaternion.identity, canvas);
